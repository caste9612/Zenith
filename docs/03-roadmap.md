...
- **Qualità — Test:** 🔶 **in corso** — runner headless + CI attivi (`npm run test:ci`, workflow `test.yml`); **49 test verdi** su metrics, format, **portfolio.service** (PMC/P&L), **quote.service** (staleness/FX/failed), **fx.provider** (Frankfurter+fallback) e **patrimonio netto** (`core/balance/net-worth`). *Mancano:* rate-limit `minIntervalMs` (mock timer), casi limite, test dei componenti. Piano in `docs/08-testing.md`.
