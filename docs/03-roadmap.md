...
- **Qualità — Test:** 🔶 **in corso** — runner headless + CI attivi (`npm run test:ci`, workflow `test.yml`); 38 test verdi su metrics, format, **portfolio.service** (PMC/P&L) e **quote.service** (staleness/FX/failed). *Mancano:* `fx.provider`, snapshot/patrimonio netto, casi limite. Piano in `docs/08-testing.md`.
