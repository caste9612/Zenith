...
- **Qualità — Test:** 🔶 **in corso** — runner headless + CI attivi (`npm run test:ci`, workflow `test.yml`); **53 test verdi** su metrics, format, **portfolio.service** (PMC/P&L), **quote.service** (staleness/FX/failed/rate-limit), **fx.provider** (Frankfurter+fallback), **patrimonio netto** (`core/balance/net-worth`) e un primo **test di componente** (AllocationPie). *Mancano:* casi limite e altri test dei componenti. Piano in `docs/08-testing.md`.
