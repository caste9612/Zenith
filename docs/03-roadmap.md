...
- **Qualità — Test:** 🔶 **in corso** — runner headless + CI attivi (`npm run test:ci`, workflow `test.yml`); 43 test verdi su metrics, format, **portfolio.service** (PMC/P&L), **quote.service** (staleness/FX/failed) e **patrimonio netto** (`core/balance/net-worth`). *Mancano:* `fx.provider`, casi limite, test dei componenti. Piano in `docs/08-testing.md`.
