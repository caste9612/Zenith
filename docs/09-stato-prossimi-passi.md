...
2. **Ampliare la suite** secondo la checklist di `docs/08-testing.md`:
   ...
   - ✅ `core/quotes/quote.service.ts` — **staleness**, conversione **EUR**, lista `failed`, selezione provider e **rate-limit `minIntervalMs`** (distanziamento, test a timer reali) → `quote.service.spec.ts`.
   - ✅ `core/quotes/fx.provider.ts` — Frankfurter + fallback `open.er-api` + `null`, con spy su `window.fetch` → `fx.provider.spec.ts`.
   - ✅ **Primo test di componente** — `AllocationPieComponent` (percentuali/fette dall'input) → `allocation-pie.spec.ts` (TestBed + `setInput`).
