...
- **Cash flow:** flusso di cassa **mensile** (entrate/uscite/risparmio) dei redditi di **Antonio**; pagina dedicata (collezione `cashFlow`), alimentata dall'import dell'Excel **e** dall'editor "Nuovo mese". Distinto dai depositi/prelievi conto per conto (fuori ambito).
- **Tasso di risparmio:** quanto cresce il **patrimonio netto** mese su mese, in **percentuale** (`ΔNW / NW_precedente`) o in **valore assoluto** (€). Basato sul **patrimonio**, non su entrate−uscite, così un **trasferimento** tra conti non sembra una spesa. In dashboard, filtrabile per **intestatario** (Nucleo/Antonio/Michela) e per **anno**. *(Sostituisce il vecchio `saved/income` del cash flow.)*
- **Tassazione (netto sul lordo):** quanto del reddito **lordo** resta **netto**, cioè `netto/lordo` cumulato sull'anno (il complemento sono tasse e contributi). Sezione "Tassazione per anno" del Cash flow; `netRate` è derivato (non salvato).
- **Registro accessi:** elenco dei **login** (data, dispositivo/browser) salvato a ogni accesso con credenziali (collezione `accessLog`), visibile in **Impostazioni**; serve a notare accessi non riconosciuti. Rilevamento **in-app**, senza email lato server (vincolo Spark — vedi `05`).
